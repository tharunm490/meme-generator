...
os.makedirs(FONTS_DIR, exist_ok=True) # Ensure fonts directory exists

# Imgflip API endpoint for getting popular memes
IMGFLIP_GET_MEMES_URL = "https://api.imgflip.com/get_memes"

# Define coding questions and answers, now categorized by skill level
# Updated Intermediate and Expert questions to be more coding-focused, easy, and funny.
CODING_QUESTIONS = {
    "python": {
        "beginner": [
...
def get_random_meme_template():
    """Fetches a random meme template from Imgflip API."""
    try:
        response = requests.get(IMGFLIP_GET_MEMES_URL)
        ...
        if data and data['success'] and data['data'] and data['data']['memes']:
            ...
            print("Error: Could not fetch meme templates from Imgflip API.")
            return None
...
def create_meme_image(text, filename="generated_meme.png"):
    """
    Generates a meme image by downloading a random template from Imgflip
    ...
    try:
        ...
        img_response = requests.get(meme_template_url)
        img_response.raise_for_status()
        img_data = io.BytesIO(img_response.content)
        img = Image.open(img_data).convert("RGB") # Ensure RGB mode for consistency
