            ...
            MEME_TEXTS = {
                "beginner": [
                    ...
                    "Just learned 'Hello World' and now I'm a full-stack developer.",
                    "When you copy-paste code and it actually works."
                ],
                "intermediate": [
                    ...
                    "My code is self-documenting... said no one ever.",
                    "When the bug disappears after you show it to someone."
                ],
                "expert": [
                    ...
                    "My code is so optimized, it runs before you even click run.",
                    "When you solve a complex problem with a single line of code."
                ]
            }
            ...
            def generate_audio_from_text(text, filename_prefix="audio", lang='en'):
                """
                Generates an MP3 audio file from text using gTTS and saves it.
                Returns the URL for the generated audio file.
                """
                try:
                    tts = gTTS(text=text, lang=lang, slow=False)
                    # Create a unique filename to avoid conflicts and browser caching issues
                    audio_filename = f"{filename_prefix}_{uuid.uuid4()}.mp3"
                    audio_path = os.path.join(MUSIC_DIR, audio_filename)
                    tts.save(audio_path)
                    return url_for('static', filename=f'music/{audio_filename}')
            ...
            def get_random_meme_template():
                """
                Selects a random meme template from the local 'curated_memes' directory.
                """
                try:
                    meme_files = [f for f in os.listdir(CURATED_MEMES_DIR) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.gif'))]
                    if not meme_files:
                        print(f"Error: No meme images found in {CURATED_MEMES_DIR}. Please add some images.")
                        return None
                    
                    random_meme_filename = random.choice(meme_files)
                    return url_for('static', filename=f'images/curated_memes/{random_meme_filename}')
                except Exception as e:
                    print(f"Error selecting local meme template: {e}")
                    return None

            def create_meme_image(text, filename="generated_meme.png"):
                """
                Generates a meme image by loading a random local template
                and overlaying the given text using Pillow.
                The image will be saved to static/images/.
                """
                try:
                    meme_template_url = get_random_meme_template()
                    if not meme_template_url:
                        print("Falling back to placeholder as no meme template URL was obtained.")
                        return url_for('static', filename='images/placeholder.png')

                    # Construct the local path from the URL_for
                    # This assumes Flask's static URL is directly mapping to the file system
                    # We need to remove the leading '/' for os.path.join to work correctly
                    local_image_path = os.path.join(app.root_path, meme_template_url.lstrip('/'))

                    img = Image.open(local_image_path).convert("RGB") # Ensure RGB mode for consistency
                    ...
                    # This is a basic implementation; for perfect meme text placement,
                    # you'd need to know the specific text box coordinates for each template.
                    lines = []
                    words = text.split()
                    current_line = []
                    for word in words:
            ...
            def index():
                ...
                # Pass all questions to the template, so JS can pick based on language AND level
            ...
            def generate_meme():
                ...
                self_assessed_skill_level = request.form.get('skill_level') # New: get self-assessed level
                user_answer = request.form.get('answer', '').strip().lower()

                meme_text = "Something went wrong. Try again!"
                final_skill_level = "beginner" # Default if anything goes wrong
                ...
                if selected_language in CODING_QUESTIONS and self_assessed_skill_level in CODING_QUESTIONS[selected_language]:
                    ...
                    # Find the specific question that was displayed to the user based on the selected language and skill level
                    for q_info in questions_for_level:
                        if user_answer == q_info['correct_answer'].lower():
                            correct_answer_found = True
                            break

                    if correct_answer_found:
                        # If they answered correctly, their final skill level for the meme is their self-assessed level
                        final_skill_level = self_assessed_skill_level
                        meme_text = random.choice(MEME_TEXTS[final_skill_level])
                    else:
                        # If they answered incorrectly, their final skill level for the meme is 'beginner'
                        final_skill_level = "beginner"
                        meme_text = random.choice(MEME_TEXTS[final_skill_level])
                else:
                    meme_text = "Please select a valid coding language and skill level."

                # Generate audio based on the final_skill_level
                # Randomly choose between English ('en') and Hindi ('hi')
                audio_lang = random.choice(['en', 'hi'])
                audio_texts_for_level = SKILL_LEVEL_AUDIO_TEXTS.get(final_skill_level, {})
                audio_text = random.choice(audio_texts_for_level.get(audio_lang, ["Enjoy your meme!"])) # Fallback text

                generated_audio_url = generate_audio_from_text(audio_text, filename_prefix=f"{final_skill_level}_{audio_lang}_audio", lang=audio_lang)
                ...
                return render_template('meme.html',
                                       ...
                                       background_music_url=generated_audio_url, # Now points to generated audio
                                       skill_level_display=final_skill_level.capitalize()) # Pass the final skill level for display

            @app.route('/static/music/<path:filename>')
            def serve_music(filename):
                """Serves music files from the static/music directory."""
                # Vercel's static file serving handles this directly from the static folder
                # This route might not be strictly necessary if Vercel serves static files from /static/
                # but keeping it for robustness or if you have specific needs.
            ...
            # The if __name__ == '__main__': app.run(debug=True) block is removed for Vercel
